  ...
  return (
    <div className="h-screen w-full overflow-y-auto">
      {receiverId.includes(session?.user.uid as string) && (
        <>
          {sender.filter(send => send.id !== session?.user.uid).map(send => (
            <>
              <div
                key={send.id}
      ...
      {receiver.filter(item => item.id !== session?.user.uid).map(receive => (
        <>
          <div
            key={receive.id}
            onClick={() => selectUser(receive)}
            className={`flex cursor-pointer items-center justify-between border-b-2 border-gray-400 border-opacity-50 transition-all ease duration-500   ${darkMode ? 'from-gray-700 p-3 hover:bg-gradient-to-r' : 'from-gray-200 p-3 hover:bg-gradient-to-r '} ${receive.id === session?.user.uid ? 'hidden' : 'block'}`}
          >
            <div className="flex items-center space-x-2">
              <Image
                src={receive.image}
